...
基于 Nextflow DSL2 的生产级鸟枪法宏基因组学工作流：物种分类、功能谱分析与
MAG 重建。

**项目状态：** Phase 0–17 已完成（输入 → 预处理 → read-based ∥ 组装 → MAG →
质控 → 去冗余 → 分类 → 基因预测 → 注释 → 丰度 → 整合 → 报告），Phase 18
（文档）进行中。

> **如实声明验证范围：** 预处理 / 组装 / 比对 / 分箱 / 基因预测 / 丰度 /
> 整合 / MultiQC 已真实运行验证（合成测试数据）；read-based、CheckM2、
> dRep、GTDB-Tk、功能注释五支依赖数 GB 级数据库，本机不可用 —— 已通过
> stub-run 验证通道拓扑、参数守卫与输出结构，**真实数据库执行未验证**。
> 容器路径未实机验证（开发机无容器引擎）。详见下文 [Limitations](#limitations)。
...
重建、质控、去冗余、分类、基因预测、功能注释、丰度），最终整合为核心
结果表并生成 MultiQC 汇总报告。
...
MAG 链内 QC → 去冗余 → 分类串联，基因预测 / 注释 / 丰度并行。详细流程见
...
GTDB-Tk/dRep/Prodigal/DIAMOND/eggNOG-mapper/RGI/CoverM/MultiQC），约 8 GB，
用于无 profile 的本地运行（各 process 从 PATH 取工具）。
...
**不静默降级、不伪造产物**。完整清单（体量/获取方式/Phase 归属）见
...
两条核心命令（Phase 16，均实测通过且 `-resume` 全命中）：

```bash
# 测试 B — stub 全流程 (54/54 tasks, 无真实工具执行, 无数据库依赖)
nextflow run main.nf -profile test -stub-run --batch_id p16_stub \
...
# 测试 A — 真实全流程 (32/32 tasks, 需 nf-meta 环境; read-based 与
# 数据库依赖阶段被 skip —— 数据库本机不可用, 见 Limitations)
nextflow run main.nf -profile test --batch_id p16_real \
...
0.57479572）已与 Phase 7/13 实测值交叉核对一致。

逐 Phase 测试矩阵（✅ 真实 / ✅ stub / ⚠️ database-dependent）见
[STATUS.md](STATUS.md)。
...
| `docker` | `conf/docker.config` | quay.io/biocontainers 镜像（tag 存在性已核验；开发机无引擎，未实机运行） |
| `singularity` | `conf/singularity.config` | Apptainer/Singularity（配置解析通过；stub 模式亦预拉镜像，需引擎） |
| `slurm` | `conf/slurm.config` | SLURM 调度（仅配置解析级验证，开发机无 sbatch） |
| `test` | `conf/test.config` | 内置合成数据 + dummy DB 门卫 + pathway 夹具 |

Profile 可叠加：`-profile test,docker -stub-run` 实测通过（54/54，stub 不拉
镜像故无需引擎）。

## 可重复性

- **版本钉死策略**：`environment.yml` 开发环境全部工具钉版（nextflow 钉
  26.04.4 —— 本项目 Phase 1–17 全部验证的实际运行版本）；每个 process 另有
...
- **`-resume` 幂等**：聚合输入用 collectFile(sort) + toSortedList 保证 task
  hash 确定，stub 54/54 与真实 32/32 均实测 cached 全命中。注意 Nextflow
